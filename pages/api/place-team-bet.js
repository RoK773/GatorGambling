...
export default async function handler(req, res) {
    ...
    try {
        ...
        const normalizedPick = {
            teamId: String(incomingPick.teamId || '').trim() || null,
            country: String(incomingPick.country || '').trim() || 'Unknown Team',
            record: String(incomingPick.record || '').trim() || '--',
            outcome: String(incomingPick.outcome || '').trim() || '--',
            range: String(incomingPick.range || '').trim() || '--',
            points: incomingPick.points ?? null,
        ...
        await users.updateOne(
            { username: normalizedUsername },
            {
                ...
                $push: { team_picks: normalizedPick },
            },
        );
        ...
        console.error('Team bet placement failed:', error);
        return res.status(500).json({ error: 'Failed to place team bet.' });
    }
}
